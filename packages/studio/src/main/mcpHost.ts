...
import type { McpToolInfo, McpConfigSnippet, StudioSettings } from '../shared/types';
...
export async function callMcpTool(toolName: string, args: Record<string, unknown>): Promise<string> {
    try {
        switch (toolName) {
            ...
            default:
                return JSON.stringify({ error: `Tool '${toolName}' not yet implemented in Studio MCP host` });
        }
    } catch (err) {
 ...
 * agent's configuration to connect to Studio's MCP server.
 */
export function generateMcpConfigSnippet(format: string): McpConfigSnippet {
    const settings = loadSettings();
    const studioPath = path.join(process.cwd(), 'packages', 'studio');

    if (format === 'claude') {
        const config = {
            mcpServers: {
                'ansible-studio': buildTransportConfig(settings, studioPath),
            },
        };
        return {
            format: 'claude',
    ...
    const config = buildTransportConfig(settings, studioPath);
    return {
        format: 'generic',
...
function buildTransportConfig(settings: StudioSettings, studioPath: string): Record<string, unknown> {
    switch (settings.mcpTransport) {
        case 'socket':
            return {
                transport: 'socket',
                socketPath: settings.mcpSocketPath ?? path.join(os.tmpdir(), 'ansible-studio-mcp.sock'),
            };
        case 'sse':
            return {
                transport: 'sse',
        ...
        default:
            return {
                command: 'node',
                args: [path.join(studioPath, 'dist', 'mcp-stdio-proxy.js')],
            };
    }
}
