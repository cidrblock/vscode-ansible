...
import type { StudioSettings, McpToolInfo, ServiceStatus, ColorScheme, AbbenayStatus, AbbenayModelInfo } from '../../shared/types';
...
export function useSettingsState(onColorSchemeChange?: (scheme: ColorScheme) => void): SettingsState {
...
function renderAiCategory(state: SettingsState): React.ReactNode {
    ...
    return (
        <fieldset style={styles.group}>
            ...
            {abbenayStatus?.error && (
                <div style={{ ...styles.helpText, color: 'var(--studio-error, #e55)' }}>
                    {abbenayStatus.error}
                </div>
            )}
...
function renderMcpCategory(state: SettingsState): React.ReactNode {
    ...
    return (
        <fieldset style={styles.group}>
            ...
            <select style={styles.input} value={settings.mcpTransport} onChange={(e) => update({ mcpTransport: e.target.value as StudioSettings['mcpTransport'] })}>
            ...
            {settings.mcpTransport === 'socket' && (
                <>
                    ...
                    <input style={styles.input} type="text" value={settings.mcpSocketPath ?? ''} onChange={(e) => update({ mcpSocketPath: e.target.value || null })} placeholder="/tmp/ansible-studio-mcp.sock" />
                </>
            )}
...
const styles: Record<string, React.CSSProperties | ((...args: unknown[]) => React.CSSProperties)> = {
    ...
    legend: { display: 'flex', alignItems: 'center', gap: '10px', fontSize: 'var(--studio-font-size-sm)', fontWeight: 600, color: 'var(--studio-text-primary)', marginBottom: '4px' },
    label: { fontSize: 'var(--studio-font-size-xs)', color: 'var(--studio-text-secondary)', marginTop: '4px' },
    input: {
        padding: '6px 10px', background: 'var(--studio-bg-tertiary)', border: '1px solid var(--studio-border)',
        borderRadius: 'var(--studio-radius-sm)', fontSize: 'var(--studio-font-size-sm)', color: 'var(--studio-text-primary)', outline: 'none',
    },
    checkLabel: { display: 'flex', alignItems: 'center', gap: '6px', fontSize: 'var(--studio-font-size-sm)', color: 'var(--studio-text-secondary)', cursor: 'pointer' },
    statusBadge: ((running: boolean) => ({
        display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '1px 8px',
        borderRadius: '10px', fontSize: 'var(--studio-font-size-xs)', fontWeight: 400,
        background: running ? 'var(--studio-success-subtle)' : 'var(--studio-bg-tertiary)',
        color: running ? 'var(--studio-success)' : 'var(--studio-text-tertiary)',
    })) as unknown as React.CSSProperties,
    subSection: { marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '4px' },
    subLabel: { display: 'flex', alignItems: 'center', gap: '8px', fontSize: 'var(--studio-font-size-xs)', fontWeight: 500, color: 'var(--studio-text-secondary)' },
    hint: { fontWeight: 400, color: 'var(--studio-text-tertiary)' },
    toolList: {
        display: 'flex', flexDirection: 'column', gap: '1px', maxHeight: '200px', overflowY: 'auto' as const,
        padding: '4px 0', background: 'var(--studio-bg-tertiary)', borderRadius: 'var(--studio-radius-sm)',
    },
    toolRow: { display: 'flex', alignItems: 'center', gap: '6px', padding: '3px 8px', fontSize: 'var(--studio-font-size-xs)', color: 'var(--studio-text-secondary)', cursor: 'pointer' },
    toolName: { fontFamily: 'var(--studio-font-mono)', fontSize: '11px' },
    snippetRow: { display: 'flex', gap: '6px', alignItems: 'center' },
    snippetSelect: {
        padding: '5px 8px', background: 'var(--studio-bg-tertiary)', border: '1px solid var(--studio-border)',
        borderRadius: 'var(--studio-radius-sm)', fontSize: 'var(--studio-font-size-sm)', color: 'var(--studio-text-primary)',
    },
    snippetContainer: { position: 'relative' as const, marginTop: '4px' },
    snippet: {
        padding: '10px', background: 'var(--studio-bg-primary)', border: '1px solid var(--studio-border)',
        borderRadius: 'var(--studio-radius-sm)', fontSize: '11px', fontFamily: 'var(--studio-font-mono)',
        color: 'var(--studio-text-secondary)', whiteSpace: 'pre-wrap' as const, maxHeight: '200px',
        overflowY: 'auto' as const, userSelect: 'text',
    },
    copyBtn: {
        position: 'absolute' as const, top: '6px', right: '6px',
        padding: '2px 8px', background: 'var(--studio-bg-hover)', borderRadius: 'var(--studio-radius-sm)',
        fontSize: 'var(--studio-font-size-xs)', color: 'var(--studio-text-secondary)',
    },
    actionBtn: {
        alignSelf: 'flex-start', padding: '5px 12px', background: 'var(--studio-bg-hover)',
        borderRadius: 'var(--studio-radius-sm)', fontSize: 'var(--studio-font-size-sm)',
        color: 'var(--studio-text-secondary)', marginTop: '4px',
    },
    helpText: { fontSize: 'var(--studio-font-size-xs)', color: 'var(--studio-text-tertiary)', lineHeight: 1.5, marginTop: '4px' },
    actions: { display: 'flex', alignItems: 'center', gap: '10px' },
    saveBtn: {
        padding: '7px 20px', background: 'var(--studio-text-primary)', color: 'var(--studio-text-inverse)',
        borderRadius: 'var(--studio-radius-sm)', fontWeight: 500, fontSize: 'var(--studio-font-size-sm)',
    },
    savedMsg: { fontSize: 'var(--studio-font-size-xs)', color: 'var(--studio-success)' },
    schemeRow: { display: 'flex', gap: '4px' },
    schemeBtn: {
        padding: '5px 16px', borderRadius: 'var(--studio-radius-sm)', fontSize: 'var(--studio-font-size-sm)',
        background: 'var(--studio-bg-tertiary)', color: 'var(--studio-text-secondary)',
        border: '1px solid var(--studio-border)',
    },
    schemeBtnActive: {
        background: 'var(--studio-text-primary)', color: 'var(--studio-text-inverse)',
        borderColor: 'var(--studio-text-primary)',
    },
    muted: { color: 'var(--studio-text-tertiary)', fontSize: 'var(--studio-font-size-sm)', padding: '24px' },
};
