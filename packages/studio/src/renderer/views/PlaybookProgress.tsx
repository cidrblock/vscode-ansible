...
function TaskStatusIcon({ results }: { results: HostResult[] }): React.JSX.Element {
    ...
    if (hasFailed) return <span style={{ ...styles.statusIcon, color: 'var(--studio-error)' }}>✗</span>;
    if (hasChanged) return <span style={{ ...styles.statusIcon, color: 'var(--studio-warning)' }}>●</span>;
    if (allSkipped) return <span style={{ ...styles.statusIcon, color: 'var(--studio-text-tertiary)' }}>◍</span>;
    return <span style={{ ...styles.statusIcon, color: 'var(--studio-success)' }}>✓</span>;
}

function StatusDot({ status }: { status: HostResult['status'] }): React.JSX.Element {
    const colorMap: Record<string, string> = {
        ok: 'var(--studio-success)',
        changed: 'var(--studio-warning)',
        failed: 'var(--studio-error)',
        skipped: 'var(--studio-text-tertiary)',
        unreachable: 'var(--studio-error)',
    };
    return (
        <span
            style={{
                ...
                height: '8px',
                borderRadius: '50%',
                background: colorMap[status] ?? 'var(--studio-text-tertiary)',
                flexShrink: 0,
            }}
        />
    );
            ...
            <div style={styles.statsGrid}>
                {Object.entries(stats).map(([host, s]) => (
                    <div key={host} style={styles.statsRow}>
                        ...
                        <span style={{ color: 'var(--studio-success)' }}>ok={s.ok}</span>
                        <span style={{ color: 'var(--studio-warning)' }}>
                            changed={s.changed}
                        </span>
...
const styles: Record<string, React.CSSProperties> = {
    ...
    handlerBadge: {
        padding: '1px 6px',
        borderRadius: '3px',
        background: 'var(--studio-info-subtle)',
